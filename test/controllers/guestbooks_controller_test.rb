...
class GuestbooksControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create guestbook" do
    assert_difference('Guestbook.count') do
      post guestbooks_url, params: { guestbook: { comment: @guestbook.comment, name: @guestbook.name } }
    end
  ...
  test "should update guestbook" do
    patch guestbook_url(@guestbook), params: { guestbook: { comment: @guestbook.comment, name: @guestbook.name } }
